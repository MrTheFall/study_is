...
create table if not exists orders (
    ...
    payment_method    varchar(32), -- cash / card / online (chosen method; may be null for legacy rows)
    ...
    constraint chk_orders_payment_method_enum check (payment_method is null or lower(payment_method) in ('cash','card','online')),
    constraint chk_orders_cooking_duration_nonneg check (cooking_duration_seconds is null or cooking_duration_seconds >= 0),
    ...
    constraint fk_orders_client
        ...
        on delete restrict,
    constraint fk_orders_created_by_employee
        foreign key (created_by_employee_id) references employees(id)
        on delete set null,
    constraint fk_orders_accepted_by_employee
        foreign key (accepted_by_employee_id) references employees(id)
        on delete set null,
...
alter table orders add column if not exists created_by_employee_id integer;
alter table orders add column if not exists accepted_by_employee_id integer;
...
alter table orders add column if not exists cooking_duration_seconds integer;

do $$
begin
    alter table orders
        add constraint fk_orders_created_by_employee
            foreign key (created_by_employee_id) references employees(id)
            on delete set null;
exception
...
begin
    alter table orders
        add constraint fk_orders_accepted_by_employee
            foreign key (accepted_by_employee_id) references employees(id)
            on delete set null;
exception
    when duplicate_object then null;
...
begin
    alter table orders
        add constraint chk_orders_payment_method_enum
            check (payment_method is null or lower(payment_method) in ('cash','card','online'));
exception
    when duplicate_object then null;
end $$;
...
begin
    alter table orders
        add constraint chk_orders_cooking_duration_nonneg
            check (cooking_duration_seconds is null or cooking_duration_seconds >= 0);
exception
    when duplicate_object then null;
end $$;

create index if not exists idx_orders_client_id on orders(client_id);
create index if not exists idx_orders_created_by_employee_id on orders(created_by_employee_id);
create index if not exists idx_orders_accepted_by_employee_id on orders(accepted_by_employee_id);
...
create table if not exists inventory_transactions (
    ...
    reason        text,
    source        varchar(32)    not null default 'manual', -- manual / order / seed / etc
    employee_id   integer,
    order_id      integer,
    created_at    timestamp      not null default now(),
    constraint chk_inventory_transactions_delta_nonzero check (delta <> 0),
    constraint fk_inventory_transactions_ingredient
        foreign key (ingredient_id) references ingredients(id)
        on delete restrict,
    constraint fk_inventory_transactions_employee
        foreign key (employee_id) references employees(id)
        on delete set null,
...
create index if not exists idx_inventory_transactions_employee_id on inventory_transactions(employee_id);
...
create table if not exists order_status_history (
    ...
    constraint chk_order_status_history_status_enum check (
        lower(from_status) in ('pending','confirmed','preparing','ready','delivering','delivered','completed','cancelled')
        and lower(to_status) in ('pending','confirmed','preparing','ready','delivering','delivered','completed','cancelled')
    ),
...
create table if not exists employee_action_logs (
    ...
    details     text,
    created_at  timestamp not null default now(),
    constraint fk_employee_action_logs_employee
        foreign key (employee_id) references employees(id)
        on delete set null,
    constraint fk_employee_action_logs_order
        foreign key (order_id) references orders(id)
        on delete set null
);
create index if not exists idx_employee_action_logs_employee_id on employee_action_logs(employee_id);
create index if not exists idx_employee_action_logs_order_id on employee_action_logs(order_id);
create index if not exists idx_employee_action_logs_created_at on employee_action_logs(created_at);
create index if not exists idx_employee_action_logs_action on employee_action_logs(action);
...
create table if not exists online_payment_sessions (
    ...
    constraint fk_online_payment_sessions_order
        foreign key (order_id) references orders(id)
        on delete cascade
);
create index if not exists idx_online_payment_sessions_order_id on online_payment_sessions(order_id);
create unique index if not exists uq_online_payment_sessions_bank_tx on online_payment_sessions(bank_transaction_id);

create table if not exists bank_payment_sessions (
    id              uuid primary key,
    order_id        integer        not null,
    amount          numeric(14,2)  not null,
    status          varchar(32)    not null,
...
create index if not exists idx_bank_payment_sessions_order_id on bank_payment_sessions(order_id);
...
create table if not exists report_views (
    ...
    constraint fk_report_views_employee
        foreign key (employee_id) references employees(id)
        on delete set null
);
create index if not exists idx_report_views_employee_id on report_views(employee_id);
...
create table if not exists salary_payments (
    ...
    note        text,
    constraint chk_salary_payments_amount_nonneg check (amount >= 0),
...
begin
    ...
    if new.status is distinct from old.status then
        if lower(new.status) = 'preparing' then
            new.preparing_at := coalesce(new.preparing_at, now());
        end if;
        if lower(new.status) = 'ready' then
            new.ready_at := coalesce(new.ready_at, now());
            if new.preparing_at is not null then
                new.cooking_duration_seconds := greatest(0, extract(epoch from (new.ready_at - new.preparing_at))::int);
            end if;
        end if;
        if lower(new.status) = 'cancelled' then
            new.cooking_duration_seconds := null;
        end if;
    end if;
    return new;
end;$$;
...
begin
    if new.status is distinct from old.status then
        ...
        values (new.id, lower(old.status), lower(new.status), now());
    end if;
    return new;
end;$$;
...
-- Синхронизация последовательностей с реальными данными
do $$
declare
    max_id integer;
begin

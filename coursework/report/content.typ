...
= #strong[Архитектура и реализация:]

Мы реализовали монолитную CRM-систему для ресторана «Красти Крабс» на Spring Boot 4. Сервер построен по слоистой архитектуре: REST-контроллеры принимают запросы, сервисы содержат бизнес-логику, репозитории работают с PostgreSQL через JPA. API описывается OpenAPI, доступ защищен JWT-аутентификацией и ролевой моделью (клиент, кассир, повар, управляющий). В доменной модели реализованы клиенты, заказы, позиции меню, ингредиенты, склад, сотрудники, смены, платежи и отзывы. Развертывание обеспечено Docker и Docker Compose, интеграция с фронтендом на React осуществляется через REST.

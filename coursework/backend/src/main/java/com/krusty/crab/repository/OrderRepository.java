...
public interface OrderRepository extends JpaRepository<Order, Integer> {
    ...
    long countOwnedByClient(@Param("clientId") Integer clientId, @Param("orderIds") List<Integer> orderIds);
    ...
    boolean existsByCourier_IdAndStatusInAndIdNot(
            Integer courierId, List<com.krusty.crab.entity.enums.OrderStatus> statuses, Integer orderId);

    @Query(
            value = "SELECT place_order(:clientId, :type, :deliveryAddress, :paymentMethod, CAST(:items AS jsonb))",
            nativeQuery = true)
    ...
    void callUpdateOrderStatus(@Param("orderId") Integer orderId, @Param("newStatus") String newStatus);

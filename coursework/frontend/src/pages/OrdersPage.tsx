...
export function OrdersPage() {
  ...
  const employeeFilteredOrders = useMemo(() => {
    ...
    const filtered = orders.filter((order) => {
      ...
      const itemsHay =
        items && items.length > 0 ? items.map((it) => (it.name || `#${it.menuItemId}`)).join(' ').toLowerCase() : '';
  ...
  const submitClientPaymentOrSwitch = async () => {
    ...
    try {
      ...
      const sanitizedCardNumber = clientCardData.number.replace(/\s+/g, '');
      ...
      if (!/^[0-9]{16}$/.test(sanitizedCardNumber)) {
        setClientPayError('Введите корректный номер карты (16 цифр)');
        return;
      }
      if (!/^(0[1-9]|1[0-2])\/\d{2}$/.test(sanitizedExpiry)) {
        setClientPayError('Срок действия в формате ММ/ГГ');
        return;
      }
  ...
  return (
    <>
      ...
      {error && (
        <RetryAlert message={error} onRetry={reload} />
      )}
              ...
              {!isClientUser && (
                ...
                const statusLabel = getOrderStatusLabel(statusStr);
	              ...
	              const canAssignCourier = (isManager() || isCashier())
	                && isDeliveryOrder
	                && statusStr !== 'cancelled'
	                && statusStr !== 'completed'
	                && statusStr !== 'delivered';
                  ...
                  : statusStr === 'completed');
		                ...
		                >
		                  ...
		                  <CardContent>
		                    <div className="space-y-2">
		                      <p><strong>Тип:</strong> {getOrderTypeLabel(order.type)}</p>
                      ...
                      {canAssignCourier && (
                        <div className="max-w-md">
                          ...
                          <Select
                            ...
                            onChange={async (e) => {
                              ...
                              try {
                                ...
                                toast.error(getApiErrorMessage(error, 'Ошибка назначения курьера'), { title: 'Ошибка' });
                              }
                            }}
                          >
                          ...
                          <p className={`mt-1 text-xs ${courierRequiredNow && !hasCourier ? 'text-red-600' : 'text-gray-500'}`}>
                                ...
                                return (
                                  <li key={step} className="flex items-center justify-between gap-3">
                                    <div className="flex items-center gap-2 min-w-0">
                                      ...
                                      <span className={`text-sm truncate ${textClass}`}>{getOrderStatusLabel(step)}</span>
                                    </div>
                                    {ts ? (
                    ...
                    {(canEmployeeProcessPayment || canClientPayOnline || canEmployeeConfirm || canEmployeeCancel || canClientCancel || canLeaveReview || canUpdateStatus) && (
                      <div className="mt-4 space-y-2">
                        ...
                        {canClientPayOnline && (
                          <Button onClick={() => openClientPaymentDialog(order)}>
                            Оплатить онлайн
                          </Button>
                        )}
                        {canEmployeeConfirm && (
                        ...
                        {canEmployeeCancel && (
                          <Button
                            variant="destructive"
                            onClick={() => openEmployeeCancel(order)}
                          >
                            Отменить
                          </Button>
                        )}
      ...
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Если оплата не проходит — попробуйте ещё раз, выберите оплату при получении или отмените заказ.
            </DialogDescription>
          </DialogHeader>
            ...
            {clientPayMethod === PaymentMethod.Online && (
              <div className="space-y-2">
                <Input
                  ...
                  onChange={(e) => setClientCardData((prev) => ({ ...prev, number: e.target.value }))}
                  disabled={clientPaySubmitting}
                />
                <div className="flex gap-2">
                  <Input
                    ...
                    onChange={(e) => setClientCardData((prev) => ({ ...prev, expiry: e.target.value }))}
                    disabled={clientPaySubmitting}
                  />
                  <Input
                    placeholder="CVV"
                    value={clientCardData.cvv}
                    onChange={(e) => setClientCardData((prev) => ({ ...prev, cvv: e.target.value }))}
                    disabled={clientPaySubmitting}
                  />
                </div>
                <p className="text-xs text-gray-500">
                  Для симуляции отказа платежа введите номер карты 0000 0000 0000 0000.
                </p>
              </div>
            )}
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={cancelClientPayOrder} disabled={clientPaySubmitting || !clientPayOrder?.id}>
              Отменить заказ
            </Button>
            <Button onClick={submitClientPaymentOrSwitch} disabled={clientPaySubmitting || !clientPayOrder?.id}>
              {clientPaySubmitting
                ? (clientPayMethod === PaymentMethod.Online ? 'Оплата...' : 'Сохранение...')
                : (clientPayMethod === PaymentMethod.Online ? 'Оплатить' : 'Сохранить')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
                ...
                {[1, 2, 3, 4, 5].map((r) => (
                  <button
                    ...
                    className={`w-10 h-10 rounded-full ${
                      r <= reviewRating ? 'bg-yellow-400' : 'bg-gray-200'
                    }`}
                    aria-label={`Оценка ${r}`}
                  >
                    ⭐

...
= #strong[Диаграмма классов:]
#align(center)[#image("arch_2.png", width: 100%)]

= #strong[Некорректные формулировки требований:]

- #strong[UR-001] «Система должна обеспечивать выполнение операции оформления заказа не более чем за 3 минуты.»  
  Не указан сценарий и метод измерения, результат зависит от навыков пользователя. Мы его не реализовали.  
  #strong[Как правильно:] «В юзабилити‑тесте на 10 пользователях не менее 90% завершают сценарий “Оформить заказ с доставкой” ≤3 мин.»

- #strong[SEC-004] «Использовать актуальные версии сторонних библиотек и зависимостей.»  
  Формулировка не измерима (нет критерия “актуальности”) и не определяет способ контроля. Мы его не реализовали.  
  #strong[Как правильно:] «В CI выполняется SCA‑сканирование; сборка блокируется при CVE high/critical; зависимости обновляются не реже 1 раза в квартал.»

= #strong[Нереализованные требования:]
#set enum(numbering: "1.")
+ Нет входа через Яндекс ID. В целом, требование избыточно, так как есть вход по номеру и почте. Критичность: 2/5.
+ Нет гарантий выполнения операции оформления заказа за 3 минуты. Это тяжело измерить и зависит от опыта пользователя. Критичность: 1/5.
+ Отсутствует программа лояльности. Критичность: 3/5.
+ Отсутствуют пользовательские предпочтения. Требование немного избыточно, ресторан ориентирован на фирменную еду. Критичность: 2/5.
+ Нет журнала действий, фиксируются только статусы заказов. Критичность: 3/5.
+ Отсутствуют мониторинг и метрики SLI/SLO/SLA.

= #strong[Правки:]

После того как подсветили несоответствие документации, был реализован журнал действий сотрудников. Добавлены сущность и таблица `employee_action_logs`, сервис логирования и эндпоинт `GET /audit/logs` с фильтрами по сотруднику, заказу, типу действия и периоду. Журнал фиксирует ключевые операции: создание и смену статусов заказов, изменение способа оплаты и назначение курьера, управление меню, складом, сменами, персоналом, курьерами, а также обработку платежей и выплат зарплаты.

= #strong[Вывод]

В ходе работы реализована монолитная CRM-система для ресторана «Красти Крабс» на Spring Boot 4. Архитектура разделена на слои: REST-контроллеры принимают запросы и выполняют валидацию, сервисы инкапсулируют бизнес-логику, репозитории обеспечивают доступ к данным в PostgreSQL через JPA. API описано в OpenAPI, а доступ к операциям ограничен JWT-аутентификацией и ролевой моделью для клиентов и сотрудников.

Система покрывает ключевые процессы ресторана и включает модули управления клиентами, заказами, меню, складом ингредиентов, оплатами, кухней, персоналом и сменами, курьерами, отзывами, а также аналитикой и финансовыми отчетами. Для критичных операций используются функции и триггеры в базе данных: автоматическая фиксация статусов и времени приготовления, пересчет итогов заказа, валидация платежей, аудит смены статусов. Производительные запросы усилены индексами под очереди кухни, историю заказов клиентов и отчеты по оплатам.

Развертывание предусмотрено через Docker и Docker Compose, а интеграция с клиентским приложением на React осуществляется по REST. Система демонстрирует рабочую реализацию основных бизнес-процессов, при этом ограничения и нереализованные требования отдельно зафиксированы в разделе «Нереализованные требования».

Дополнительно мы смоделировали реальный процесс онлайн‑оплаты через банк с поддержкой 3‑D Secure: запуск платежной сессии, получение ссылки на ACS, обработка callback/notification и финализация платежа на стороне сервера.

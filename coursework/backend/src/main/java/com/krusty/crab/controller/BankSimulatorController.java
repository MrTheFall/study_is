...
public class BankSimulatorController {
    ...
    public ResponseEntity<BankPaymentInitResponse> createPayment(
        ...
        BankPaymentInitResponse response = bankSimulatorService.initiatePayment(
            request,
            signature,
            timestamp,
            nonce,
    ...
    public ResponseEntity<String> submitAcs(
        ...
        BankPaymentStatus finalStatus = session.getForceFailure()
            ? BankPaymentStatus.DECLINED
            : BankPaymentStatus.APPROVED;

        bankSimulatorService.updateStatus(session, finalStatus, finalStatus == BankPaymentStatus.DECLINED ? "declined" : null);
    ...
    private String renderAcsPage(BankPaymentSession session, String error) {
        String amount = session.getAmount() != null ? session.getAmount().stripTrailingZeros().toPlainString() : "0";
        String last4 = session.getCardLast4() != null ? session.getCardLast4() : "0000";
        String createdAt = session.getCreatedAt() != null ? session.getCreatedAt().format(DATE_FORMAT) : "";
        String expiresAt = session.getExpiresAt() != null ? session.getExpiresAt().format(DATE_FORMAT) : "";
        ...
        sb.append("<!doctype html><html lang=\"ru\"><head><meta charset=\"utf-8\">")
            ...
            .append(".card{max-width:520px;margin:40px auto;background:#fff;border-radius:16px;padding:24px;box-shadow:0 12px 24px rgba(0,0,0,0.08);}")
            ...
            .append(".error{background:#fee2e2;color:#b91c1c;padding:10px 12px;border-radius:8px;font-size:13px;margin-bottom:12px;}")
        ...
        sb.append("<div class=\"row\"><span class=\"label\">Мерчант</span><span>Krusty Krab</span></div>")
            ...
            .append("<button class=\"primary\" type=\"submit\" name=\"action\" value=\"approve\">Подтвердить</button>")
            .append("</form>")
            ...
            .append("</form>")
            .append("<div class=\"hint\">Тестовый код: ").append(session.getOtpCode()).append("</div>")
    ...
    private String renderAutoPostForm(String returnUrl, BankPaymentResultPayload payload, String message) {
        ...
        sb.append("<!doctype html><html lang=\"ru\"><head><meta charset=\"utf-8\">")
            ...
            .append("<p>").append(safeMessage).append("</p>")
            .append("<form id=\"returnForm\" method=\"post\" action=\"").append(returnUrl).append("\">")
            .append("<input type=\"hidden\" name=\"transactionId\" value=\"").append(payload.transactionId()).append("\"/>")
            .append("<input type=\"hidden\" name=\"orderId\" value=\"").append(payload.orderId()).append("\"/>")
            .append("<input type=\"hidden\" name=\"amount\" value=\"").append(payload.amount().stripTrailingZeros().toPlainString()).append("\"/>")
            .append("<input type=\"hidden\" name=\"status\" value=\"").append(payload.status()).append("\"/>")
            .append("<input type=\"hidden\" name=\"timestamp\" value=\"").append(payload.timestamp()).append("\"/>")
            .append("<input type=\"hidden\" name=\"nonce\" value=\"").append(payload.nonce()).append("\"/>")
            .append("<input type=\"hidden\" name=\"signature\" value=\"").append(payload.signature()).append("\"/>")
            .append("</form>")
